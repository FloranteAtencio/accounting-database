...
--BEGIN
  ...
  --  RETURN QUERY
    ...
    SELECT 
        ...
        t.Description,
        c.Account,
        CASE WHEN d.Journal = 'true' THEN d.Amount ELSE 0 END AS Debit,
        CASE WHEN d.Journal = 'false' THEN d.Amount ELSE 0 END AS Credit
...
--BEGIN
  ...
  --  RETURN QUERY
    ...
    SELECT 
        c.Account,
        c.Type,
        SUM(CASE WHEN d.Journal = 'true' THEN d.Amount ELSE 0 END) AS TotalDebit,
        SUM(CASE WHEN d.Journal = 'false' THEN d.Amount ELSE 0 END) AS TotalCredit
    FROM Finance.Transactions t
...
--BEGIN
  ...
  -- RETURN QUERY
    ...
    SELECT 
        c.Type,
        c.Account,
        SUM(CASE WHEN d.Journal = 'true' THEN d.Amount ELSE 0 END) -
        SUM(CASE WHEN d.Journal = 'false' THEN d.Amount ELSE 0 END) AS NetAmount
    FROM Finance.Charts c
    JOIN Finance.Journals d ON c.ChartID = d.ChartID
...
--   RETURN QUERY
    ...
    SELECT 
        ...
        c.Account,
        SUM(CASE WHEN d.Journal = 'true' THEN d.Amount ELSE 0 END) -
        SUM(CASE WHEN d.Journal = 'false' THEN d.Amount ELSE 0 END) AS Balance
    FROM Finance.Charts c
    JOIN Finance.Journals d ON c.ChartID = d.ChartID    
    JOIN Finance.Transactions t ON t.TransactionID = d.TransactionID
